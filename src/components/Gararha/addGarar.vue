  ...
  <div class="grid lg:grid-cols-12 gap-4">
    ...
    >
      ...
      >
        ...
        >
          از این بخش می توانید قرار را اضافه کنید
        </p>
      </div>
    ...
    <aside class="lg:col-span-3 col-span-12">
      <div class="mt-2 border border-gray-200 rounded-lg card">
        <div class="flex items-center w-full p-4 border-b border-gray-200">
          ...
          ><span>افزودن قرار جدید</span>
        </div>
        <div class="block p-4">
          <button
            type="button"
            @click="add_new_appointment(1)"
          ...
          >
            ...
            ><span
              ...
              ><span class="w-full">
                ...
                <span v-else>افزودن قرار</span>
              </span></span
            >
          </button>
          <button
            type="button"
            @click="add_new_appointment(0)"
          ...
          >
            ...
            ><span
              ...
              ><span class="w-full"> 
                <div class="lds-ring" v-if="draftLoading == true"><div></div><div></div><div></div><div></div></div>
                <span v-else>پیش نویس</span>
              </span></span
            >
          </button>
...
export default {
  ...
  methods: {
    ...
    async add_new_appointment(status) {
      if(status == 0) {
        this.appointData.status = 0
      }
      const validateResult = this.validateData()
      if(validateResult == true){
        this.loading = status == 1 ? true : false
        this.draftLoading = status == 0 ? true : false
        const result = await this.$store.dispatch("add_new_appointment" , this.appointData)
    ...
    validateData() {
      ...
      }else if(this.appointData.image == ""){
        return "تصویر شاخص الزامی میباشد"
      }else {
        return true
      }
    },
    ...
    clearData() {
      this.appointData.title = ""
      this.appointData.end_date =  ""
      this.appointData.appointment_time =  ""
      this.appointData.location =  ""
      this.appointData.description =  ""
      this.appointData.summary_description =  ""
      this.appointData.is_main =  false
      this.appointData.slug =  ""
      this.appointData.image =  ""
      this.appointmentImgSrc = ""
      this.$refs.appointment_image_input.value = ""
