<template>
  ...
  const add_user = async () => {
    if (validate_user_info() == true) {
      ...
      if(userData.mobile != "") {
        userData.mobile = "0" + parseInt(userData.mobile);
      }

      const result = await store.dispatch("create_user_account", {
      ...
      if (result.status == 200) {
        loading.value = false;
        showSwal("پیغام موفقیت آمیز" , `کاربر جدید با نام ${userData.name} ${userData.family} اضافه شد`, "success");
        clear_data();
      } else {
        loading.value = false;
  ...
  const validate_user_info = () => {
    if (userData.password == "") {
      showSwal("خطایی رخ داد" , "گذرواژه نمیتونه خالی باشه", "error");
      return false;
    } else if (userData.name == "") {
      showSwal("خطایی رخ داد" , "نام نمیتونه خالی باشه", "error");
      return false;
    } else if (userData.family == "") {
      showSwal("خطایی رخ داد" , "نام خانوادگی نمیتونه خالی باشه", "error");
      return false;
      ...
      return false;
    } else if (userData.mobile == "" && userData.email == "") {
      showSwal("خطایی رخ داد" , "لطفا شماره موبایل و یا ایمیل را وارد کنید", "error");
      return false;
    } else {
      return true;
    }
