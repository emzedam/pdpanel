<template>
  ...
  >
    <div class="col-span-1 row-span-1 p-6 border rounded-lg">
      <div class="flex items-start content-between justify-between">
        <div class="content-left">
          <span class="text-sm text-gray-500">تعداد نوشته ها</span>
          <div class="flex items-center my-1">
            <h4 class="mb-0 ml-2 font-bold">{{ mostanadInformation.mostanadatCount }}</h4>
          </div>
          <span class="text-xs text-gray-500">مجموع مستندات منتشر شده</span>
        </div>
        <span
    ...
    <div class="col-span-1 row-span-1 p-6 border rounded-lg">
      <div class="flex items-start content-between justify-between">
        <div class="content-left">
          <span class="text-sm text-gray-500">تعداد پیش نویس</span>
          <div class="flex items-center my-1">
            <h4 class="mb-0 ml-2 font-bold">{{ mostanadInformation.draftsCount }}</h4>
          </div>
  ...
  <div class="w-full pb-6">
    <div class="overflow-hidden">
      <div class="gap-4 lg:grid lg:grid-cols-12">
        <dataset
          v-slot="{ ds }"
          :ds-data="mostanadInformation.mostanadList"
          :ds-sortby="sortBy"
          :ds-sort-as="{}"
          :ds-search-in="['title']"
          :ds-search-as="{}"
        >
          <div class="col-span-12 font-fa">
            <div
              class="mt-2 mb-6 bg-white border border-gray-200 blog-info rounded-xl"
            >
              <div
              ...
              >
                ...
                >
                  از این بخش می توانید مستندات را مدیریت یا حذف نمایید
                </p>
              </div>
              ...
              <div class="py-8 border-b px-7 card-header">
                ...
                >
                    ...
                    <RouterLink to="/add-mostanad">
                      ...
                      >
                        ...
                        ><span
                          ...
                          ></i
                          ><span class="w-full">افزودن مستند جدید</span></span
                        >
                      </button>
                    </RouterLink>
                  </div>
                  ...
                  <div class="w-full">
                    <export-excel
                      :data="mostanadInformation.mostanadList"
                      :fields="excelFields"
                      title="لیست مستندات حامیان پت و من"
                      :name="`mostanadat.xls`"
                                  ...
                                  >
                                    <div class="flex px-6 py-2 text-right">
                                      <div class="flex items-center">
                                        <img
                                          :src="`${globalUrl}/storage/mostanadat/${row.image}`"
                        ...
                        >
                          <template #default="{ row, rowIndex }">
                            <tr class="border-b border-gray-200 divide-x">
                              ...
                              <td class="p-2 text-right mx-auto flex items-center justify-center">
                                <video width="150" height="110" controls>
                                  <source :src="`${globalUrl}/storage/mostanadat/${row.video}`">
                                </video>
                              </td>
                              ...
                              <td class="p-2 text-center">
                                ...
                                >
                                  <label
                                    ...
                                    ><input
                                      type="checkbox"
                                      :value="row.is_special == 1 ? 1 : 0"
                                      :checked="row.is_special == 1 ? true: false"
                                      @click="do_change_special_state(row.is_special , row.id , rowIndex)"
                              ...
                              >
                                ...
                                >
                                  ...
                                  <RouterLink :to="`/edit-mostanad/${row.id}`">
                                    <div
                                      class="w-4 transform cursor-pointer hover:text-gray-500 hover:scale-110"
                                    >
                                      <i
...
export default {
  ...
  emits: ["updateMostanad" , "removeMostanadList"],
  components: {
    Dataset,
    DatasetInfo,
    DatasetItem,
  ...
  methods: {
    ...
    do_change_special_state(state , id , index) {
      if(state == 0) {
        Swal.fire({
          title: "هشدار",
          text: "آیا میخواهید این مستند به لیست ویژه ها بپیوندد؟",
          icon: "warning",
          ...
          if (result.isConfirmed) {
            const result = await this.chnage_special_state(1 , id)
            if(result.status == 200){
              this.showSwal("پیغام موفقیت آمیز" , result.message , "success")
              this.$emit("updateMostanad" , {state: result.result , id: id})
            }else{
        ...
        Swal.fire({
          title: "هشدار",
          text: "آیا میخواهید این مستند از لیست ویژه ها حذف شود؟",
          icon: "warning",
          showCancelButton: true,
          confirmButtonColor: "#9d2c48",
          cancelButtonColor: "#555",
          ...
          if (result.isConfirmed) {
            const result = await this.chnage_special_state(0 , id)
            if(result.status == 200){
              this.showSwal("پیغام موفقیت آمیز" , result.message , "success")
              this.$emit("updateMostanad" , {state: result.result , id: id})
            }else{
    ...
    async chnage_special_state(state , id) {
      const result = await this.$store.dispatch("change_mostanad_special_state" , {id: id, state: state})
      return result
    },
      ...
      if(this.deletableIds.length != 0){
        if(value != ""){
          Swal.fire({
            ...
            if (result.isConfirmed) {
              const result = await this.$store.dispatch("delete_list_of_mostanad" , {ids: this.deletableIds})
              if(result.status == 200){
                this.showSwal("پیغام موفقیت آمیز" , result.message , "success")
                this.$emit("removeMostanadList" , {ids: result.result})
              }else {
                this.showSwal("پیغام خطا" , result.message , "error")
              }
            }
        ...
        this.showSwal("پیغام خطا" , "لطفا مستندی را جهت حدف انتخاب کنید" , "error")
      }
    },
    async deleteMostanad(id) {
      Swal.fire({
            title: "هشدار",
            text: "آیا از انجام این اقدام مطمعن هستید؟",
            icon: "warning",
            ...
            if (result.isConfirmed) {
              const result = await this.$store.dispatch("delete_mostanad" , {id: id})
              if(result.status == 200){
                this.showSwal("پیغام موفقیت آمیز" , result.message , "success")
                this.$emit("removeMostanadList" , {ids: [id]})
              }else {
                this.showSwal("پیغام خطا" , result.message , "error")
              }
            }
