<template>
  <div class="dropdown">
    ...
    <div class="relative flex items-center">
      <button
        type="button"
        class="text-gray-500 border border-gray-300 focus:ring-1 focus:outline-none focus:ring-hamian font-medium rounded-lg text-sm px-4 py-4 text-center inline-flex items-center justify-between w-[250px]"
        @click="toggleFocus"
      >
        <i
      ...
      <button
        type="button"
        @click="$emit('bulkActionDelete' , value)"
        class="mr-2 text-white bg-hamian font-medium rounded-lg text-sm px-4 py-4 block text-center items-center"
      >
        پذیرش
      </button>
    </div>
    <div
      class="categories absolute top-18 z-10 mt-2 bg-white border rounded-lg w-full p-3 max-w-[250px]"
      v-if="focused"
    >
      <ul>
        <li
