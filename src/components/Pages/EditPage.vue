  ...
  <div class="grid lg:grid-cols-12 gap-4">
    ...
    >
      ...
      >
        ...
        >
          از این بخش می توانید توضیحات صفحه را ویرایش کنید
        </p>
      </div>
                    ...
                    <div class="relative col-span-6 mt-4">
                      ...
                      <div class="mt-1 rounded-md">
                        ...
                        >
                          ...
                          <TagInput ref="tagInputRef" :modelValue="pageInfo.keywords" v-model="pageData.keywords"></TagInput>
                        </div>
                      </div>
                    </div>
                  </div>
...
export default {
  ...
  async mounted(){
    ...
    if(this.pageInfo != null){
      this.pageData.id = this.pageInfo.id
      this.pageData.title = this.pageInfo.title
      this.pageData.slug = this.generateSlug(this.pageInfo.title);
      this.$refs.ckeditorRef.config.initialData = this.pageInfo.content
      this.pageData.content = this.pageInfo.content
      this.pageData.meta_title = this.pageInfo.meta_title
      this.pageData.meta_description = this.pageInfo.meta_description
      this.pageData.keywords = this.pageInfo.keywords != null ? this.pageInfo.keywords.split(',') : []
      this.seoImgSrc  = this.pageInfo.seo_image != null ? `${this.globalUrl}/storage/pages/seo/${this.pageInfo.seo_image}`: ''
      this.seoImgSrc2 = this.pageInfo.image != null ? `${this.globalUrl}/storage/pages/${this.pageInfo.image}`: ''
    }
  ...
  methods: {
    ...
    async editPageData() {
      ...
      if(validate == true){
        this.loading = true
        const result = await this.$store.dispatch("edit_page_data" , this.pageData)
