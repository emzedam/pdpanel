<template>
  ...
  >
    <div class="col-span-1 row-span-1 p-6 border rounded-lg">
      <div class="flex items-start content-between justify-between">
        <div class="content-left">
          <span class="text-sm text-gray-500">همه صفحات</span>
          <div class="flex items-center my-1">
            <h4 class="mb-0 ml-2 font-bold">{{ pagesInformation.pagesCount }}</h4>
          </div>
          <span class="text-xs text-gray-500">مجموع صفحه ایجاد شده</span>
        </div>
        <span
    ...
    <div class="col-span-1 row-span-1 p-6 border rounded-lg">
      <div class="flex items-start content-between justify-between">
        <div class="content-left">
          ...
          <div class="flex items-center my-1">
            <h4 class="mb-0 ml-2 font-bold">{{ pagesInformation.draftsCount }}</h4>
          </div>
    ...
    <div class="col-span-1 row-span-1 p-6 border rounded-lg">
      <div class="flex items-start content-between justify-between">
        <div class="content-left">
          <span class="text-sm text-gray-500">منتشر شده</span>
          <div class="flex items-center my-1">
            <h4 class="mb-0 ml-2 font-bold">{{ pagesInformation.releasedCount }}</h4>
          </div>
          <span class="text-xs text-gray-500">مجموع صفحات منتشر شده</span>
        </div>
        <span
  ...
  <div class="w-full pb-6">
    <div class="overflow-hidden">
      <div class="gap-4 lg:grid lg:grid-cols-12">
        <dataset
          v-slot="{ ds }"
          :ds-data="pagesInformation.pagesList"
          :ds-sortby="sortBy"
          :ds-sort-as="{}"
          :ds-search-in="['title']"
          :ds-search-as="{}"
        >
          <div class="col-span-12 font-fa">
            <div
              class="mt-2 mb-6 bg-white border border-gray-200 page-info rounded-xl"
            >
              <div
              ...
              >
                ...
                >
                  از این بخش می توانید صفحات را مدیریت یا حذف نمایید
                </p>
              </div>
              ...
              <div class="py-8 border-b px-7 card-header">
                ...
                >
                    ...
                    <RouterLink to="/add-page">
                      ...
                      >
                        ...
                        ><span
                          ...
                          ></i
                          ><span class="w-full">افزودن صفحه جدید</span></span
                        >
                      </button>
                    </RouterLink>
                  </div>
                  ...
                  <div class="w-full">
                    <export-excel
                      :data="pagesInformation.pagesList"
                      :fields="excelFields"
                      title="لیست صفحات حامیان پت و من"
                      :name="`pages.xls`"
                                  ...
                                  >
                                    <div class="flex px-6 py-2 text-right">
                                      <div class="flex items-center">
                                        <img
                                          :src="`${globalUrl}/storage/pages/${row.image}`"
                        ...
                        >
                          <template #default="{ row, rowIndex }">
                            <tr class="border-b border-gray-200 divide-x">
                              ...
                              <td
                                class="p-2 text-center text-[15px] ltr text-gray-400"
                              >
                                /{{ row.slug }}
                              </td>
                              ...
                              >
                                ...
                                >
                                  ...
                                  <RouterLink :to="`/edit-page/${row.id}`">
                                    <div
                                      class="w-4 transform cursor-pointer hover:text-gray-500 hover:scale-110"
                                    >
                                      <i
  ...
  methods: {
        ...
        if(value != ""){
          Swal.fire({
            ...
            if (result.isConfirmed) {
              const result = await this.$store.dispatch("delete_list_of_pages" , {ids: this.deletableIds})
              if(result.status == 200){
                this.showSwal("پیغام موفقیت آمیز" , result.message , "success")
                this.$emit("removePagesList" , {ids: result.result})
              }else {
    ...
    async deletePage(id) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          const result = await this.$store.dispatch("delete_page" , {id: id})
          if(result.status == 200){
            this.showSwal("پیغام موفقیت آمیز" , result.message , "success")
            this.$emit("removePagesList" , {ids: [id]})
          }else {
            this.showSwal("پیغام خطا" , result.message , "error")
          }
        }
