<template>
    <form
      id="navbar"
      class="top-0 z-30 col-span-12 lg:col-span-4"
      @submit.prevent="category_create()"
    >
      <div class="mt-2 border border-gray-200 rounded-lg">
        <div class="flex items-center w-full p-4 border-b border-gray-200">
          <i class="pl-2 text-2xl text-hamian fa-duotone fa-grid-2-plus"></i>
          <span>افزودن دسته بندی جدید محصول</span>
        </div>
        ...
        <div class="p-4 tab-content sm:p-5">
          <div class="space-y-5">
            ...
            <div class="flex mt-2 rounded-md shadow-sm">
              <div class="relative items-stretch flex-grow focus-within:z-10">
                <button
                  ...
                  class="text-gray-500 border border-gray-300 focus:ring-1 focus:outline-none focus:border-hamian focus:ring-hamian font-medium rounded-lg text-xs px-4 py-2.5 text-center inline-flex items-center w-full justify-between"
                >
                  <i
                ...
                <div
                  class="z-10 w-full mt-2 bg-white absolute border rounded-lg"
                  :class="!categoryStatus ? 'hidden' : ''"
                > 
                  <div class="p-2">
                    <input class="w-full border-1 rounded-lg border-gray-300 outline-none" type="search" placeholder="جستجو کنید" @keyup="(e) => doSearchCat(e)" />
                  </div>
                  <ul
                    v-if="categorySearchList.length == 0"
                    class="h-80 p-3 overflow-y-auto text-sm text-gray-700"
                  ...
                  >
                    ...
                    <li
                      ...
                      :key="index"
                      @click="setParentId(item.id, item.title)"
                    >
                      <div
                        class="flex items-center w-full my-1 border rounded hover:bg-gray-100"
                      >
                      ...
                      <ul
                        class="pr-2 overflow-y-auto text-sm text-gray-700"
                        v-if="item.childs.length != 0"
                      >
                        <li
                          v-for="(child, index) in item.childs"
                          :key="index"
                          @click.stop="setParentId(child.id, child.title)"
                        >
                          <div
                            class="flex items-center w-full my-1 border rounded hover:bg-gray-100"
                          >
                          ...
                          >
                            <li
                              ...
                              :key="index"
                              @click.stop="setParentId(subchild.id, subchild.title)"
                            >
                              <div
                                class="flex items-center w-full my-1 border rounded hover:bg-gray-100"
                              >
            ...
            >
              <div class="flex items-center justify-center w-full pt-2 rounded-lg grow">
                <div
              ...
              <div class="flex items-center justify-center w-full pt-2 rounded-lg grow">
                ...
                >
                  <img
                    v-if="data.cat_img_reader_file != null"
        ...
        <div class="block p-4">
          ...
          >
            ...
            <span class="relative z-20 flex items-center justify-center w-full text-center">
              ...
              <span class="w-full">افزودن دسته محصول</span>
            </span>
          </button>
          <button
            type="button"
          ...
          >
            ...
            <span class="relative z-20 flex items-center justify-center w-full text-center">
              <i class="pl-2 text-2xl text-white fa-duotone fa-arrow-up-from-bracket"></i>
              <span class="w-full">در حال ارسال ...</span>
            </span>
          </button>
        </div>
      </div>
  ...
  export default {
    ...
    methods: {
      doSearchCat(e) {
        this.$emit('search_category', e.target.value)
      },
      async category_create() {
        ...
        if (this.data.title != null) {
          ...
          const send_data = await this.$store.dispatch("save_category", formData);
          ...
          if (send_data.status != 200) {
            ...
            this.loading = false;
            toast.success("دسته بندی مورد نظر شما با موفقیت ایجاد گردید", {
              autoClose: 3000,
              theme: "colored",
              transition: "flip",
              position: "bottom-right",
            ...
            await this.$emit("updateCategories");
            this.clearData();
          }
        } else {
          this.loading = false;
