<template>
  ...
  >
    <div class="lg:col-span-8 col-span-12 font-fa">
      <div
        class="mt-2 mb-6 bg-white border border-gray-200 product-info rounded-xl"
      >
        <div
        ...
        >
          ...
          >
            <i
              class="fa-duotone fa-grid-2 text-[20px] !w-5 !h-5 text-hamian ml-1"
            ></i
            >دسته بندی ها
          </h6>
          <p
            ...
            <div class="min-w-full align-middle">
              ...
              <div class="relative hidden lg:block Desktop-Table">
                <div class="w-full">
                  <header>
                    <ul
                      class="text-sm leading-normal text-right text-gray-600 uppercase border-t grid grid-cols-8"
                    >
                      <li
                  ...
                  >
                    <template
                      ...
                      v-if="categoryList.length != 0"
                    >
                      <ul class="border-b border-gray-200 grid grid-cols-8">
                        <li
                        ...
                        >
                          ...
                          <i
                            ...
                            :class="{
                              'fa-minus text-2xl': openedChildIds.includes(
                                'cat-' + row.id
                              ),
                              'fa-plus text-2xl': !openedChildIds.includes(
                                'cat-' + row.id
                              ),
                            }"
                          ></i>
                        ...
                        >
                          ...
                          >
                            <label
                              ...
                              ><input
                                type="checkbox"
                                :value="row.is_header == 1 ? 1 : 0"
                                :checked="row.is_header == 1 ? true : false"
                                @click="
                                  do_change_header_status(
                                    row.is_header,
                                    row.id,
                                    rowIndex
                                  )
                        ...
                        >
                          <div class="flex justify-center item-center">
                            ...
                            <RouterLink :to="`/edit-category/${row.id}`">
                              <div
                                class="w-4 ml-4 transform cursor-pointer hover:text-gray-500 hover:scale-110"
                              >
                                <i
                      ...
                      >
                        <div>
                          <div
                            class="border-b border-gray-200 grid grid-cols-8 SUB-MENU bg-gray-50"
                          >
                            <div
                            ...
                            >
                              ...
                              <i
                                ...
                                class="fa cursor-pointer"
                                :class="{
                                  'fa-minus text-2x': openedChildIds.includes(
                                    'cat-' + child.id
                                  ),
                                  'fa-plus text-2x': !openedChildIds.includes(
                                    'cat-' + child.id
                                  ),
                                }"
                              ></i>
                            ...
                            >
                              <div class="flex justify-center item-center">
                                ...
                                <RouterLink :to="`/edit-category/${child.id}`">
                                  <div
                                    class="w-4 ml-4 transform cursor-pointer hover:text-gray-500 hover:scale-110"
                                  >
                                    <i
                        ...
                        >
                          <div
                            ...
                            :key="index"
                            class="border-b border-gray-200 grid grid-cols-8 SUB-MENU bg-gray-100"
                          >
                            <li
                            ...
                            >
                              <i
                                class="fa fa-empty-set text-hamian cursor-pointer"
                              ></i>
                            </li>
...
export default {
  ...
  methods: {
    openTable(id) {
      const classCollections = Array.from(
        document.getElementsByClassName("cat-" + id)
      );
    ...
    do_change_header_status(state , id , index) {
      if(state == 0) {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            const result = await this.chnage_cat_header_status(1 , id)
            if(result.status == 200){
              this.showSwal("پیغام موفقیت آمیز" , result.message , "success")
              this.$emit("updateCategory" , {state: result.result , id: id})
            }else{
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            const result = await this.chnage_cat_header_status(0 , id)
            if(result.status == 200){
              this.showSwal("پیغام موفقیت آمیز" , result.message , "success")
              this.$emit("updateCategory" , {state: result.result , id: id})
            }else{
    ...
    async chnage_cat_header_status(state , id) {
      const result = await this.$store.dispatch("change_category_header_status" , {id: id, state: state})
      return result
    },
