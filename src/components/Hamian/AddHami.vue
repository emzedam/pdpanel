  ...
  <div class="grid lg:grid-cols-12 gap-4">
    ...
    >
      ...
      >
        ...
        >
          از این بخش می توانید حامی را اضافه کنید
        </p>
      </div>
                  ...
                  >
                    <ul class="h-[300px] overflow-y-scroll">
                      ...
                      <li
                        ...
                        v-for="(favCategory , index) in searchResFavCategories"
                        :key="index"
                        @click="selectFavCategory(favCategory.id , favCategory.name)"
                      >
                      <span class="bg-red-400 hover:bg-red-500 px-2 py-1 rounded-lg ml-2" @click.stop="delete_cat(favCategory.id , index , 'fav')">
                    ...
                    <div class="border-t mt-5 bg-gray-50 px-2 py-2">
                      <div>
                        <input
                          type="text"
                          v-model="favorite_cat_name"
                          class="border border-gray-200 w-full my-2 rounded-lg placeholder:text-sm text-gray-400 placeholder:text-gray-400/70"
                      ...
                      <button
                        @click="addFavCat()"
                        class="mb-2 w-full text-white bg-hamian border-hamian font-bold focus:outline-none justify-center mt-2 border px-3 py-2 rounded-lg flex items-center"
                      >
                        <i v-if="favLoading == false" class="fa fa-plus"></i
                        ><span class="mr-2 font-normal">
                          <div class="lds-ring" v-if="favLoading == true"><div></div><div></div><div></div><div></div></div>
                  ...
                  >
                    <ul class="h-[300px] overflow-y-scroll">
                      ...
                      <li
                        ...
                        class="py-1 relative text-gray-500 mt-2 cursor-pointer hover:bg-gray-50 px-2 rounded-md"
                        v-for="(category , index) in searchActCategories"
                        :key="index"
                        @click="selectActCategory(category.id , category.name)"
                        :class="{ selected: value === category.value }"
                      >
                        <span class="bg-red-400 hover:bg-red-500 px-2 py-1 rounded-lg ml-2" @click.stop="delete_cat(category.id , index , 'act')">
                          <i class="fa fa-trash text-white"></i>
                        </span>  
                        {{ category.name }}
                        <i
                    ...
                    <div class="border-t mt-5 bg-gray-50 px-2 py-2">
                      <div>
                        <input
                          type="text"
                          v-model="act_cat_name"
                          class="border border-gray-200 w-full my-2 rounded-lg placeholder:text-sm text-gray-400 placeholder:text-gray-400/70"
                          placeholder="مثال: پرستار حیوانات"
                        />
                      </div>
                      <button
                        @click="addActCat()"
                        class="mb-2 w-full text-white bg-hamian border-hamian font-bold focus:outline-none justify-center mt-2 border px-3 py-2 rounded-lg flex items-center"
                      >
                        <i v-if="actLoading == false" class="fa fa-plus"></i
                        ><span class="mr-2 font-normal">
                          <div class="lds-ring" v-if="actLoading == true"><div></div><div></div><div></div><div></div></div>
                          <span v-else>افزودن </span>
                        </span>
                      </button>
                    </div>
...
export default {
  ...
  methods: {
    ...
    async addNewSupporter() {
      const validateResult = this.validateData()
      if(validateResult == true){
        this.loading = true
        const result = await this.$store.dispatch("add_new_supporter" , this.hamianData)
        if(result.status == 200){
          this.loading = false
    ...
    clearData() {
        this.hamianData.fullName = ''
        this.hamianData.slug = ''
        this.hamianData.mobile = ''
        this.hamianData.faveorite_animal.ids = []
        this.hamianData.faveorite_animal.names = []
        this.hamianData.activity.ids = []   
        this.hamianData.activity.names = []  
        this.hamianData.socials = []
        this.hamianData.profile_image = ''
        this.$refs.supporter_image_input.value = ""
        this.supporterImgSrc = ''
    },
    async addFavCat() {
      if(this.favorite_cat_name != '') {
        this.favLoading = true
        const result = await this.$store.dispatch("add_fav_cat" , {cat_name: this.favorite_cat_name})
        if(result.status == 200){
          this.favLoading = false
          this.favoriteCategories = result.result
          this.searchResFavCategories = result.result
          this.favorite_cat_name = ''
        }else {
          this.favLoading = false
          this.showSwal("خطایی رخ داد !" , result.message , "error")
        }
        ...
        this.favLoading = false
        this.showSwal("خطایی اعتبارسنجی !" , "لطفا فیلد مربوطه را پر کنید" , "error")
      }
    },
    async addActCat() {
      if(this.act_cat_name != '') {
        this.actLoading = true
        const result = await this.$store.dispatch("add_act_cat" , {cat_name: this.act_cat_name})
        if(result.status == 200){
    ...
    async delete_cat(id , index , type) {
      Swal.fire({
        title: "هشدار",
        ...
        if (result.isConfirmed) {
          const result = await this.$store.dispatch("delete_supporters_cat" , {
            id: id,
            type: type
          })
